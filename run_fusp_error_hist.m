function [ ] = run_fusp_error_hist(type,subjID)

if nargin <2
    type=input('Enter type of shift [1. eh to i \n 2. eh to ah]: '); 
    if nargin<1
...
switch type
    case 1
        ...
        ramp_F1 = linspace(0,expt.shift.F1(type),shift1_n);
        ramp_F2 = linspace(0,expt.shift.F2(type),shift1_n);
        expt.condValues.F1 = [zeros(1,baseline1_n) ramp_F1 repmat(expt.shift.F1(type),1,hold_n) zeros(1,baseline2_n) repmat(expt.shift.F1(type),1,shift2_n) zeros(1,washout_n)];
        expt.condValues.F2 = [zeros(1,baseline1_n) ramp_F2 repmat(expt.shift.F2(type),1,hold_n) zeros(1,baseline2_n) repmat(expt.shift.F2(type),1,shift2_n) zeros(1,washout_n)];   
...
p.fusp_init.expr_subdir = sprintf('%s', expt.type);
