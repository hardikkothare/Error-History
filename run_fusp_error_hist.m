...
expt.snum = subjID;
expt.case=input (' Case 1 or Case 2 ? : ','s');
 if ~exist(fullfile(outputdir, subjID, expt.case))
    mkdir(fullfile(outputdir, subjID, expt.case))
end
...
% init parameters
p.fusp_init.expr_dir = fullfile(sprintf('%s', expt.snum),sprintf('%s',expt.case));
...
savefile = fullfile(outputdir,expt.snum,expt.case,'p.mat');
save(savefile,'p');
